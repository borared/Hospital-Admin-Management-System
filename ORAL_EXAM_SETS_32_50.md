...
The `Patient.setName()` method is easy to test:
...
- **Single responsibility** - only validates and sets ONE field (the patient's name)
- **Clear inputs/outputs** - takes a String, returns boolean
- **Pure logic** - checks null, emptiness, and regex pattern
- **No dependencies** - doesn't call other classes or methods
- **Deterministic** - same input always produces same output
- **Easy test cases** - can test: null, empty string, invalid characters, valid name, boundary cases
...
public void testSetName() {
    Patient patient = new Patient("123", "John", "01/01/1990", "Disease", "0123456789", "01/01/2024");
    assertTrue(patient.setName("Alice"));              // Valid name
    assertFalse(patient.setName(null));               // Null
    assertFalse(patient.setName(""));                 // Empty
    assertFalse(patient.setName("John123"));          // Has numbers
    assertFalse(patient.setName("John@"));            // Has special characters
    assertTrue(patient.setName("A"));                 // Single character
    assertTrue(patient.setName("Mary Jane Smith"));   // Space allowed
}
```

This method demonstrates single responsibility - it validates and sets ONE field, making it focused and easy to test in isolation.
